...
test("ReviewGrid renders numbered bubbles for each question", () => {
  ...
  expect(screen.getByRole("listitem", { name: /Question 1, q-1/ })).toBeDefined();
  expect(screen.getByRole("listitem", { name: /Question 2, q-2, flagged/ })).toBeDefined();
  expect(screen.getByRole("listitem", { name: /Question 3, q-3, unanswered/ })).toBeDefined();
});
...
test("ReviewGrid calls onSelectQuestion when a bubble is clicked", () => {
  ...
  fireEvent.click(screen.getByRole("listitem", { name: /Question 2, q-2, flagged/ }));
